...
const SwipeableItem = ({ children, onDelete, containerClassName = "" }: { children: React.ReactNode, onDelete: () => void, containerClassName?: string }) => {
  ...
  return (
    <div className={`relative overflow-hidden ${containerClassName}`} style={{ touchAction: 'pan-y' }}>
...
const TopBar = ({
  ...
  <div className="bg-slate-50 text-slate-900 p-5 shrink-0 flex flex-col justify-between items-start gap-2 z-10">
    <div className="flex justify-between items-center w-full mt-2">
      ...
      <div className="flex items-center gap-2">
        ...
        >
          {!isError && (
            <span
              className={`w-1.5 h-1.5 rounded-full ${syncStatus === "Cloud Synced" ? "bg-green-500 animate-pulse" : "bg-amber-500"}`}
            ></span>
          )}
...
const BottomNav = ({
  ...
  <div className="bg-white border-t border-slate-200 absolute bottom-0 w-full flex justify-between items-center px-6 py-3 z-20 pb-safe shadow-[0_-4px_12px_rgba(0,0,0,0.03)]">
    <button
      onClick={() => setActiveTab("home")}
      className={`flex flex-col items-center transition-all ${activeTab === "home" ? "text-blue-600 scale-105" : "text-slate-400 hover:text-slate-600"}`}
    >
      <Home
        size={22}
    ...
    <button
      ...
      className={`flex flex-col items-center transition-all ${activeTab === "accounts" ? "text-blue-600 scale-105" : "text-slate-400 hover:text-slate-600"}`}
    >
      <CreditCard
        size={22}
    ...
    <button
      ...
      className={`flex flex-col items-center transition-all ${activeTab === "history" ? "text-blue-600 scale-105" : "text-slate-400 hover:text-slate-600"}`}
    >
      <History
        size={22}
    ...
    <button
      ...
      className={`flex flex-col items-center transition-all ${activeTab === "budgets" ? "text-blue-600 scale-105" : "text-slate-400 hover:text-slate-600"}`}
    >
      <PieChart
        size={22}
...
const AddTransactionModal = ({
  ...
  return (
    <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-50 flex items-end justify-center p-0 z-[60]">
      <div
        className="bg-white w-full max-w-md rounded-t-[2rem] p-6 shadow-2xl animate-slide-up border border-slate-100 max-h-[85vh] overflow-y-auto"
      ...
      >
        <div className="flex justify-between items-center mb-6">
          ...
          <button
            onClick={onClose}
            className="text-slate-400 hover:text-slate-600 bg-slate-100 hover:bg-slate-200 transition rounded-full p-1.5"
          >
            <X size={18} />
          </button>
        </div>
...
export default function App() {
  ...
  const handleDeleteTransaction = async (idToDelete: number) => {
    ...
    const updatedAccounts = accounts.map(acc => {
      if (acc.name === txToDelete.account) {
        const balanceModifier = txToDelete.type === 'expense' ? txToDelete.amount : -txToDelete.amount;
    ...
    if (isFirebaseConfigured && user && db && !isError) {
      ...
      try {
        ...
        await setDoc(stateRef, { accounts: safeAccounts, transactions: updatedTransactions, loans });
      } catch (error) {
  ...
  const handleDeleteAccount = async (idToDelete: number) => {
    ...
    if (isFirebaseConfigured && user && db && !isError) {
      ...
      try {
        const safeAccounts = sanitizeAccounts(updatedAccounts);
        await setDoc(stateRef, { accounts: safeAccounts, transactions: transactions, loans });
      } catch (error) {
        console.error("Cloud delete failure:", error);
      }
    }
  ...
  const handleDeleteLoan = async (idToDelete: number) => {
    ...
    if (isFirebaseConfigured && user && db && !isError) {
      ...
      try {
        ...
        await setDoc(stateRef, { accounts: safeAccounts, transactions, loans: updatedLoans });
      } catch (error) {
        console.error("Cloud delete failure:", error);
      }
    }
  ...
  const handleAddAccount = async (e: React.FormEvent) => {
    ...
    // OPTIMISTIC UI: Close the Modal instantly!
    setNewAccName("");
    setNewAccBalance("");
    setIsNewAccountModalOpen(false);

    // Wait for the cloud to catch up in the background
    if (isFirebaseConfigured && user && db && !isError) {
      const stateRef = doc(
        db,
        "artifacts",
  ...
  const handleAddLoan = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!newLoanName || !newLoanTotal || isNaN(parseFloat(newLoanTotal))) return;

    const total = parseFloat(newLoanTotal);
    const remaining = newLoanRemaining && !isNaN(parseFloat(newLoanRemaining)) ? parseFloat(newLoanRemaining) : total;
        ...
        return (
          <div className="space-y-8">
            ...
            <div className="bg-gradient-to-br from-blue-500 to-indigo-600 rounded-[2rem] p-6 text-white shadow-lg shadow-blue-500/30 relative overflow-hidden mx-1">
              ...
              <p className="text-blue-100 text-sm font-semibold tracking-wide mb-1">
                Total Net Balance
              </p>
              ...
              <div className="flex gap-4 overflow-x-auto pb-4 pt-1 px-1 snap-x hide-scrollbar">
                {accounts.map((acc) => {
                  ...
                  return (
                    ...
                    >
                      <div
                        className={`w-12 h-12 rounded-full flex items-center justify-center mb-4 ${acc.bg || "bg-blue-50"} ${acc.color || "text-blue-500"}`}
                      >
            ...
            <div className="px-1">
              ...
              <div className="bg-white rounded-3xl shadow-[0_2px_10px_rgba(0,0,0,0.04)] border border-slate-100 overflow-hidden divide-y divide-slate-50">
               {transactions.slice(0, 4).map((tx) => (
                  <SwipeableItem key={tx.id} onDelete={() => handleDeleteTransaction(tx.id)}>
                    <div className="p-4 flex items-center justify-between hover:bg-slate-50/50 transition duration-150 bg-white">
                      <div className="flex items-center gap-4">
                        <div
                          className={`w-10 h-10 rounded-full flex items-center justify-center shrink-0 ${tx.type === "expense" ? "bg-red-50 text-red-500" : "bg-green-50 text-green-500"}`}
                        >
                          {tx.type === "expense" ? (
        ...
        return (
          <div className="space-y-6">
              ...
              {accounts.map((acc) => {
                ...
                return (
                  <SwipeableItem 
                    ...
                    containerClassName="rounded-2xl shadow-sm border border-slate-100"
                  >
                    <div className="bg-white p-5 flex items-center justify-between hover:border-slate-200 transition-all cursor-default h-full">
                      <div className="flex items-center gap-4 min-w-0">
                        <div
                          className={`w-11 h-11 rounded-xl flex items-center justify-center shrink-0 ${acc.bg || "bg-slate-50"} ${acc.color || "text-slate-600"}`}
                        >
                          <IconComponent size={22} />
                        </div>
                        <div className="min-w-0">
            ...
            {isNewAccountModalOpen && (
              <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-50 flex items-end justify-center sm:items-center p-0 z-[60]">
                <div
                  className="bg-white w-full max-w-md rounded-t-[2rem] sm:rounded-2xl p-6 shadow-2xl animate-slide-up border border-slate-100 max-h-[85vh] overflow-y-auto"
                  style={{
                ...
                >
                  ...
                  <form onSubmit={handleAddAccount} className="space-y-4 pb-10">
                    <div>
                      <label className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-1">
                        Account Label
                      </label>
                      <input
                        ...
                        className="w-full bg-slate-50 border border-slate-200 rounded-xl py-3 px-4 text-slate-900 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:bg-white transition text-sm"
                    ...
                    <div>
                      ...
                      >
                        ...
                        <option value="ewallet">
                          Electronic Wallet (Paypal, Venmo)
                        </option>
                      </select>
                    </div>
                    <button
                      type="submit"
                      className="w-full py-3.5 bg-blue-600 hover:bg-blue-700 text-white font-bold text-xs uppercase tracking-wide rounded-xl shadow-lg shadow-blue-500/20 transition-all active:scale-95 mt-4"
                    >
                      Create Account
                    </button>
                  </form>
        ...
        return (
          <div className="space-y-6">
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden divide-y divide-slate-100">
              {transactions.map((tx) => (
                <SwipeableItem key={tx.id} onDelete={() => handleDeleteTransaction(tx.id)}>
                  <div className="p-4 flex items-center justify-between hover:bg-slate-50/30 transition-colors bg-white">
                    <div className="flex items-center gap-3 min-w-0">
                      <div
                        className={`w-9 h-9 rounded-full flex items-center justify-center shrink-0 ${tx.type === "expense" ? "bg-red-50 text-red-500" : "bg-green-50 text-green-500"}`}
                      >
                        {tx.type === "expense" ? (
                          <TrendingDown size={18} />
                        ) : (
          ...
          <div className="space-y-6">
            <div className="flex justify-between items-center">
              <div>
                ...
                <p className="text-slate-500 text-xs mt-0.5">
                  Track financial IOUs & dynamic loans
                </p>
              </div>
              <button
            ...
            <div className="grid gap-3">
              {loans.map((loan) => (
                <SwipeableItem 
                  ...
                  containerClassName="rounded-2xl shadow-sm border border-slate-100"
                >
                  <div className="bg-white p-5 flex flex-col justify-center hover:border-slate-200 transition-all cursor-default h-full">
                    ...
                    <div className="w-full bg-slate-100 rounded-full h-2 overflow-hidden">
                      <div
                        className={`h-full rounded-full transition-all duration-500 ${loan.type === "borrowed" ? "bg-red-500" : "bg-green-500"}`}
                        style={{
                          width: `${((loan.total - loan.remaining) / loan.total) * 100}%`,
                        }}
                      ></div>
                    </div>
                  </div>
            ...
            {isNewLoanModalOpen && (
              <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-50 flex items-end justify-center sm:items-center p-0 z-[60]">
                ...
                >
                  ...
                  <form onSubmit={handleAddLoan} className="space-y-4 pb-10">
                    <div>
                      <label className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-1">
                        Who is this with?
                      </label>
  ...
  return (
    <div className="bg-slate-100 h-screen overflow-hidden font-sans text-slate-900 flex justify-center">
      <div className="w-full max-w-md bg-slate-50 h-full relative shadow-2xl flex flex-col border-x border-slate-200/50 overflow-hidden">
        <TopBar
          title={
            activeTab === "home"
              ? "My Dashboard"
        ...
        <main className={`p-4 flex-1 overflow-y-auto animate-fade-in hide-scrollbar ${activeTab === 'home' ? 'pb-16' : 'pb-32'}`}>
          {renderView()}
        </main>
        ...
        {!isAddModalOpen && !isNewAccountModalOpen && !isNewLoanModalOpen && (
          <button
            ...
            className="absolute bottom-8 left-1/2 -translate-x-1/2 w-14 h-14 bg-blue-600 hover:bg-blue-700 text-white rounded-full shadow-xl shadow-blue-500/30 flex items-center justify-center z-30 transition-all duration-300 ease-in-out border-4 border-slate-50"
